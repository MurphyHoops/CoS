...
Keep independent calls parallel only when they do not conflict; await mutations before dependent work. Use image(result.content.find(item => item.type === "image")) for native images. A script without text/image returns no intermediates. Nested calls still check live permissions, retain caller identity, and are recorded separately. Chat-owned operations still require exact identity. New user input arrives only with the outer result. Use direct tools when a result needs fresh model judgment. No persistent state or arbitrary yield.`;

export function codeModeInstructions(sessionWaitAvailable = false): string {
  return CODE_MODE_INSTRUCTIONS_BASE + (sessionWaitAvailable
    ? ' Prefer direct session_wait; if hidden, code mode may call it. Successful action=arm is terminal: remaining script stops and only the wait receipt returns. Never arm in Promise.all or beside a mutation.'
    : '');
}

export const CODE_MODE_INSTRUCTIONS = codeModeInstructions(false);
