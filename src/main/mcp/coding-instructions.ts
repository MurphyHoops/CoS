...
- For GitHub Actions, long background processes, or deliberate timer waits that may outlive the current provider turn, use session_wait once and finish the turn. The local CoS supervisor will monitor the registered condition and queue the continuation. Do not keep an open ChatGPT turn alive by repeatedly polling gh run view, write_stdin, sleep, or equivalent status commands after session_wait has accepted the wait.
